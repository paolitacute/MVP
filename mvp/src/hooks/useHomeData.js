...
export const useHomeData = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // 2. Fetch seller details and their store ID[cite: 13]
      ...
      // Return empty arrays early if user has no associated store yet[cite: 13]
      if (storeError || !storeData) {
        return { sellerName, allOrders: [], newOrders: [], listings: [] }; 
      }

      const storeId = storeData.id;

      // 3. Fetch Orders[cite: 13]
      ...
      // 4. Extract unique product IDs to fetch order thumbnails safely[cite: 13]
      const productIds = [
        ...new Set(
      ...
      // 5. Fetch Product Listings[cite: 13]
      ...
      // Map raw SQL orders into card structures[cite: 13]
      ...
      // Filter "New" orders specifically by matching the exact Spanish database status[cite: 13]
      const formattedNewOrders = formattedOrders.filter(
        (order) => order.status?.toLowerCase() === 'nueva'
      );

      // Map product listings into card structures[cite: 13]
